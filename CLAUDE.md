...
`agent/manager.ts` is a thin, provider-agnostic orchestrator: session state machine, DB persistence, WS dispatch, two-phase watchdog (WARN-ONLY in both phases — a 180s pre-first-byte alert for auth/network/CA diagnostics, a 90s quiet-stretch alert after; it never kills work), a force-settle escape hatch (`adapter.runTurn` is raced against a manager-owned deferred so Stop — after a 12s grace — and `/reset` always land even when an adapter promise is wedged), capability advertisement. `sendTurn()` looks up the adapter and delegates.
