...
## 22. Don't fake usage limits from token counts

**Symptom:** a request to "show Hermes' usage limits in the per-session indicator" tempts you to compute "% used" from `result.usage` tokens, or to wire the `usage_update` ACP notification ([`hermes.ts:755-760`](../../../packages/daemon/src/agent/providers/hermes.ts)).

**Why that's wrong:** `usage_update` is a **per-turn token-usage refresh**, not a subscription window — there's no cap, no reset, nothing to anchor a "% of limit" to. Tokens are cost accounting. The real limits are the **SuperGrok / X Premium+** subscription quota behind xAI OAuth, which ACP does not expose.

**Right posture:** advertise `capabilities.usageLimits = false` (badge hidden, no errors). Surfacing real limits is an **out-of-band** job — read `~/.hermes/auth.json` + call xAI's account/usage API via the optional `fetchUsageLimits?(s)` adapter hook — and is a follow-up, not v1. See [`reference/usage-limits.md`](reference/usage-limits.md) and [`docs/reference/USAGE_LIMITS.md`](../../../docs/reference/USAGE_LIMITS.md).
