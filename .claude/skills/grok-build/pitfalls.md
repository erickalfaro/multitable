...
## 5. One `grok agent stdio` child per project cwd — assume per-cwd, VERIFY whether a singleton is safe

The Hermes adapter spawns one child per project cwd because Hermes reads its own `os.getcwd()` for self-perception/context-file discovery and ignores the per-session cwd. **VERIFY whether Grok has the same constraint** — Grok takes `cwd` on `session/new` and may honor it correctly, in which case a single daemon-wide child driving many sessions is fine (the Codex model). Until verified, **default to per-cwd** (the safe choice): a child whose process cwd *is* the project root cannot misread `AGENTS.md`. Also relevant: Grok writes a **workspace-trust** prompt/record per directory (`~/.grok/workspace-trust.json`) on first run — a fresh cwd may need trust established before tools run (see §9).

## 6. Grok self-gates approvals — `code` mode runs many tools with NO prompt

Like every ACP agent, **Grok decides which tool calls emit `session/request_permission`.** In `code` mode it reads/edits/runs commands automatically; only calls Grok itself deems sensitive surface a prompt. There is **no host-side "ask me about everything" lever** over ACP (no `canUseTool`, no host hook — Grok's hooks run Grok-side). If a user reports "Grok edited a file without asking," that's Grok's gating granularity, not a MultiTable bug, and not fixable in `handleAcpPermission` (no server-request was sent). The honest mitigations are: run in `ask` (read-only) or `plan` mode, or accept Grok's model. Do **not** fake interception in the adapter — there's no tool call to intercept. (This mirrors the Hermes §10b reality; the *cause* — agent-side gating — is identical across ACP agents, but document it from Grok's own `code`/`plan`/`ask` model, not Hermes' `DANGEROUS_PATTERNS`.)
