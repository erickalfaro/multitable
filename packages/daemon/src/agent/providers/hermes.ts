...
export class HermesAdapter implements ProviderAdapter {
  ...
  readonly capabilities: ProviderCapabilities = {
    ...
    // Hermes doesn't model modes the same way Claude/Codex do. We expose the
    // baseline three so the UI's ModeBadge has something coherent to show.
  ...
  async runTurn(
    ...
    try {
      const result = await this.client.prompt({ sessionId: hermesSessionId, text });
